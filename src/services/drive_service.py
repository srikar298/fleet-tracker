...
class DriveService:
    def __init__(self):
        ...
        for d in [self.base_dir, self.flagged_dir, self.financial_dir, self.expense_dir, self.kyc_dir, self.incident_dir, self.archive_dir]:
    ...
    def flag_trip_images(self, date_str, driver_name, trip_id):
        """Copies a trip's folder to the Flagged_Review folder for easy discrepancy audits"""
        safe_driver_name = "".join([c if c.isalnum() else "_" for c in driver_name])
        source_dir = os.path.join(self.base_dir, date_str, safe_driver_name, trip_id)
        target_dir = os.path.join(self.flagged_dir, f"{date_str}_{safe_driver_name}_{trip_id}")
