...
class AdminHandler(BaseHandler):
    ...
    async def set_price_cmd(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> Any:
        ...
        # Format: /setprice <ClientID> <ClientName> <Billed> <Payout>
        parts = update.message.text.split()
        if len(parts) < 5:
            await update.message.reply_text(
                "⚠️ Usage: `/setprice <ClientID> <Client_Name> <Billed> <Payout>`\n"
                "Example: `/setprice C-XGAT Amazon 2500 1800`",
                parse_mode="Markdown"
            )
            return None
        ...
        try:
            ...
            success = self.sheets.add_client_rate(c_id, client_name, billed, payout)
            if success:
                await update.message.reply_text(
                    f"✅ Rates updated for **{c_id}** ({client_name})\n"
                    f"💰 Billing: ₹{billed}\n"
                    f"💸 Payout: ₹{payout}",
                    parse_mode="Markdown"
                )
            else:
                await update.message.reply_text("❌ Failed to update rates. Check Master_Clients sheet.")
        except ValueError:
