...
class IncidentHandler(BaseHandler):
    async def report_damage_cmd(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        vehicles = self.sheets.get_all_vehicles()
        keyboard = [
            [InlineKeyboardButton(v, callback_data=f"rep_{v}")] for v in vehicles
        ]
    ...
    async def handle_report_vehicle(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
